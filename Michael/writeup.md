...
```
We can then find the differentials.
```math
\frac{dx}{du} = \frac{b-a}{2}
\quad \Rightarrow \quad
dx = \frac{b-a}{2}\ du.
```
We combine this and get our transformation.
```math
\boxed{
...
```
Now, we can investigate how they perform on certain integrands.

## Applications and Discussion

An interesting thing to examine for a numerical method is the rate at which it converges to the true value. So, consider the function
```math
f(x) = \sin(\sqrt{100 x})
```
...
```
When we run it through some test values for Gaussian Quadrature, we obtain the following.
...
Now, we can use Gaussian Quadrature on this transformed integral.
